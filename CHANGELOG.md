...
- **Login Flow**: Resolved infinite redirection loop by ensuring user profile re-initialization on page refresh/initial load.
- **Microservice Auth**: Synchronized `JWT_SECRET` for `api-gateway` and implemented `/api/v1/users/me` in `user-service` for robust profile loading.
- **Routing**: Fixed 404 errors for `/products` and other sidebar modules by registering missing routes and creating a generic `PlaceholderPage.vue`.
- **RBAC Infrastructure**: 
    - Enriched JWT claims with standardized role names and permissions.
    - Configured `api-gateway` to propagate authorization data via `X-User-Roles` and `X-User-Permissions` headers.
    - Updated `user-service` to deliver specialized `UserResponse` with full role/permission metadata in `snake_case`.
- **Master Data UI**: Implemented `ProductsPage.vue` with functional listing, pagination, and status tracking.

## [1.2.1] - 2026-01-26

### Fixed - Frontend Accessibility
- **Container Networking**: Resolved "connection refused" between Cloudflare Tunnel and Nginx by unifying their network namespaces.
- **Nginx Recovery**: Implemented `nginx-fixed.conf` with runtime DNS resolution for improved reliability.
- **Docker Infrastructure**: Repaired container metadata corruption following an infrastructure update.
